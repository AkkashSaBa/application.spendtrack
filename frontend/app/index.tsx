...
function Dashboard({ user, onSignedOut }: { user: User; onSignedOut: () => void }) {
  ...
  const [form, setForm] = useState({ amount: "", category: "Food", note: "", type: "expense" as TxType, goalId: null as string | null });
  ...
  const openAdd = () => { setEditing(null); setForm({ amount: "", category: TRANSFERRED_CATEGORIES[0], note: "", type: "expense", goalId: null }); setEditorOpen(true); };
  const openEdit = (t: Transaction) => { setEditing(t); setForm({ amount: String(t.amount), category: t.category, note: t.note || "", type: t.type, goalId: t.goal_id ?? null }); setActionsFor(null); setEditorOpen(true); };
  ...
  const submitTransaction = async () => {
    ...
    const payload = { type: form.type, amount, category: form.category, note: form.note.trim(), date: editing?.date || new Date().toISOString().slice(0, 10), goal_id: form.type === "savings" ? form.goalId : null };
    try {
      if (editing) {
