...
# Usernames that are always promoted to admin on login/session-check, regardless of
# what's in the database. Edit this set to add/remove hardcoded admins.
ADMIN_USERNAMES = {"akkash"}
...
def _to_user_response(doc: dict[str, Any]) -> UserResponse:
    return UserResponse(
        id=doc["id"],
        username=doc.get("username") or doc["email"].split("@", 1)[0],
        email=doc["email"],
...
async def _ensure_role(doc: dict[str, Any]) -> dict[str, Any]:
    """Promote hardcoded admin usernames and backfill role/disabled on legacy users."""
    desired_role = "admin" if doc.get("username") in ADMIN_USERNAMES else (doc.get("role") or "user")
    updates: dict[str, Any] = {}
    if doc.get("role") != desired_role:
...
async def current_user(credentials: HTTPAuthorizationCredentials | None = Depends(bearer)) -> dict[str, Any]:
    ...
    user = await db.users.find_one({"id": user_id}, {"_id": 0, "id": 1, "username": 1, "email": 1, "role": 1, "disabled": 1})
    if not user:
...
# ---------- Email guardrail gate (from playbook â do NOT weaken) ----------
    ...
    for href, text in scan.anchors:
        ...
        for m in _HOSTISH.finditer(text):
            if not _same_site(m.group(1).lower(), real):
                raise ValueError(f"Anchor text {m.group(1)!r} â  real link host {real!r} (G3)")
...
async def signup(input: SignupInput):
    ...
    user = {
        ...
        "role": "admin" if input.username in ADMIN_USERNAMES else "user",
        "disabled": False,
    }
